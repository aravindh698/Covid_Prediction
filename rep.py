...
def predict():
    ...
    if request.method == 'POST':
        ...
        Runnynose = request.form['Runny_nose']
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('INSERT INTO Prediction2 VALUES (%s,%s,%s,%s,%s,%s,%s,%s)',(contactwithcovidpatient,Age,Bodypain,Severity,Difficultyinbreathing,Runnynose,output,session['id']))
        mysql.connection.commit()
        cursor.close()
        
    return render_template('index2.html', prediction_text='Chances of covid [0-no and 1-yes] and Your result is {}'.format(output))
